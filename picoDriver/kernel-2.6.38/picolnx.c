...
// alterei a linha abaixo para o kernel 2.6.38
long pico_ioctl ( struct file* file, unsigned int cmd, unsigned long arg);
...
long pico_ioctl (struct file*  file, unsigned int cmd, unsigned long arg)
  ...
  // alterei a linha acima para o kernel-2.6.38
  //port_ptr = &port_info [MINOR (file->f_dentry->d_inode->i_rdev)];
  // alterei a linha acima para o kernel-3.19
  port_ptr = &port_info [MINOR (file->f_path.dentry->d_inode->i_rdev)];
  switch (cmd)
    {
