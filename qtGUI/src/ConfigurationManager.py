...
def debug(s):
    return;print u'DEBUG: ' + s

def info(s):
    return;print u'INFO: ' + s
...
class ConfigurationManager(object):
    ...
    def save(self):
        ...
        for name, opt in self.options.items():
            self.settings.setValue(name, opt['getter'](opt['widget']))
    ...
    def __getitem__(self, option):
        ...
        return unicode(self.settings.value(option).toString())
    ...
    def __setitem__(self, option, value):
        ...
        if opt['values'] and unicode(value) not in opt['values']:
            opt['setter'](opt['widget'], self.settings.value(option))
            
            debug(u'Valor inválido para a opção "%s": "%s". '
                  u'Os valores permitidos são %s' % (
                        option, unicode(value), opt['values']))
            return
    ...
    def backup(self, category='main'):
        u'''Faz um backup das configurações atuais da categoria.
    ...
    def restore(self, category='main'):
        u'''Restaura as configurações aos valores presentes no backup.'''
    ...
    def restrict(self, option, values):
        ...
        if (opt['widget'] is not None) and (not opt['set_items']):
            debug(u'O widget da opção "%s" não suporta a operação "set_items".' % option)
            return False
        
        if opt['widget'] is None:
            info(u'A opção "%s" ainda não tem um widget.' % option)
            return True
    ...
    def _updateStoredValue(self, option):
        ...
        if opt['values'] and unicode(val.toString()) not in opt['values']:
    ...
    def addCustomWidget(self, widget, specs, option='', category='main'):
        ...
        if not option:
            debug(u'O nome da opção não foi especificado')
        ...
        info(u'Adicionando a opção "%s"...' % option)
