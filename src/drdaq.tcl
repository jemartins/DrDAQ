...
proc ::tick {} {
global startMoment stopped TMax

if {$stopped} {return}
after 1 tick
set elapsedMS [expr {[clock clicks -milliseconds] - $startMoment}]
set elapsedMS [expr {double($elapsedMS)/1000}]

Cronometro config -text [format "%.2f" $elapsedMS]
...
#
#puts "elapsedMS= $elapsedMS ms"
}
#############################################################################
## Procedure:  InitCron

proc ::InitCron {} {
global widget stopped startMoment intervalo cont ncontagem led graf
...
proc ::ColetaDados {} {
global widget stopped Passo Sensor1 Sensor2 AutoEscala Modo1 Modo2
global xmax ymax maior_divx menor_divx maior_divy menor_divy
global nsensores File


# Quais os parametros para os sensores
# 
QualSensor

# Lendo os Módulos grace.so e drdad.so
# 
load grace.so grace
load drdaq.so drdaq
...
if {(($File == -1) || ($File == 0))} {
    tk_messageBox -icon info -message "Falha ao abrir o drdaq.   Você Precisa Resolver Esse Problema Antes de Continuar" -type ok -parent .drdaqgrace
    Parar configure -state disabled
    Iniciar configure -state normal
    return
}

# Se xmgrace não esta aberto, Abre o xmgrace e inicializa a aquisição de dados
# e configura titulo e subtitulo
 
# Esse Truque é meio bizarro
...
# Coloca alguns parametros para o gráfico
# 
if {($AutoEscala == 0)} {
    SetEscala $xmax $ymax $maior_divx $menor_divx $maior_divy $menor_divy
    SetSimbolo $nsensores
} else {
    SetSimbolo $nsensores
}
...
proc ::Cron {} {
global startMoment stopped intervalo TMax Delta_T AutoEscala cont led
global File nsensores set_value1 set_value2 nmodo1 nmodo2
...
set elapsedMS [expr {[clock clicks -milliseconds] - $startMoment}]
set elapsedMS [expr {double($elapsedMS)/1000}]
set elapseInt [expr {[clock clicks -milliseconds] - $intervalo}]
...
if { (([expr {double($elapseInt)/1000}]) >= $Delta_T) || ($elapsedMS == 0)} {
    ...
    set intervalo [clock clicks -milliseconds]

    # Lendo dados dos sensores
    if {($nsensores == 2)} {
        
        set y1 [LeDados $File $set_value1 $nmodo1]
        set y2 [LeDados $File $set_value2 $nmodo2]
        
        Medidor1 config -text [format "%.2f" $y1]
        Medidor2 config -text [format "%.2f" $y2]
    ...
    # Plotando os pontos     
    PlotaGrace $elapsedMS $y1 $elapsedMS $y2 $nsensores
...
# Sempre lê um ponto a mais ;-(
if {$elapsedMS >= [expr {$TMax + $Delta_T}]} {
    Parar configure -state disabled
    Iniciar configure -state normal
    set stopped 1    
}
...
proc ::Contador {} {
global startMoment stopped intervalo Delta_T AutoEscala ncont ncontagem cont
global File nsensores set_value1 set_value2 nmodo1 nmodo2 led
...
if {(([expr {double($elapseInt)/1000}]) >= $Delta_T) || ($elapsedMS == 0)} {
    ...
    Cronometro config -text [format "%d" $ncontagem]
    set ncontagem [expr {$ncontagem + 1}]
...
proc ::Legendas {} {
global widget Sensor1 Sensor2 Modo1 Modo2 Passo

GracePrintf "title \"drdaq & grace\""
if {($Sensor1 == 1) && ($Sensor2 == 1)} {
    GracePrintf "subtitle \"$Modo1 x $Passo e $Modo2 x $Passo\""
} elseif {($Sensor1 == 1)} {
...
proc ::KillSet {} {
global widget nsensores graf AutoEscala nsensores
...
if {($graf > 0)} {
    ...
    if {($nsensores == 1)} {    
        ...
        GracePrintf "write g0.s0 file \".\/tmp\/grafico$graf.dat\""
        ...
        GracePrintf "redraw"
        set graf [expr {$graf + 1}]
        GracePrintf "write g0.s1 file \".\/tmp\/grafico$graf.dat\""
...
if {$Conector2 ne "Interno"} {
    ...
    checkbutton $top.lab63.che66 \
        ...
        -disabledforeground #a1a1a1 -foreground black -highlightcolor black \
        -text check -variable "$top\::che66" 
